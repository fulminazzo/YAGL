...
class FullscreenGUIParserTest {
    ...
    void testSaveAndLoad() throws IOException {
        FullscreenGUI expected = GUI.newFullscreenGUI(GUIType.BREWING, 9)
                .setTitle("Full Size GUI");
        GUIParserTest.setupContents(expected);

        File file = new File("build/resources/test/fullscreen-gui.yml");
    ...
    void testLoadWithNullContents() throws Exception {
        ...
        GUI gui = new FullscreenGUIParser().load(configuration, "path");

        assertInstanceOf(FullscreenGUI.class, gui);
    }
    ...
    void testGUITypeNotSpecified() {
        ...
        IllegalArgumentException ex = assertThrowsExactly(IllegalArgumentException.class, () ->
                new FullscreenGUIParser().load(configuration, "path")
        );
    ...
    void testLoadNullSection() throws Exception {
        assertNull(new FullscreenGUIParser().load(mock(IConfiguration.class), "path"));
    }

    @Test
    void testSaveNull() {
        assertDoesNotThrow(() -> new FullscreenGUIParser().dump(mock(IConfiguration.class), "path", null));
    }
